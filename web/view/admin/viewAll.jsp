...
<body>
    ...
    <c:choose>
        <c:when test="${plan.status.statusId == 1}">
            <a href="edit?id=${plan.plid}&check=1" class="btn btn-success mb-2">Approve</a>
            <br>
            <a href="edit?id=${plan.plid}&check=0" class="btn btn-danger">Cancel</a>
        </c:when>
        <c:when test="${plan.status.statusId != 1}">
            <a href="../manager/viewAtt?id=${plan.plid}" class="btn btn-dark mb-2">Chi phí</a>
        </c:when>
    </c:choose>
</td>
                    </tr>
